...
n = 2 # number of buyers
c, d = 0.8+np.random.normal(0, 1, (n, )), np.random.normal(2, 1, (n, )) # generate some buyers each buyer vi is characterized by its (c[i], d[i])
# c, d = np.array([-3, -1, 2, 4])/5, np.array([11, 8, 13, 15])
c, d = np.array([1,2]), np.array([1,5])
...
# g_ave = np.zeros((n,))
T = 10000
sigma = np.min(B**3) # strong convexity parameter
sum_t = 0
for t in range(1, T+1):
    # compute full subgrad
    gg_full = subgrad_full(beta)
    # projected subgradient step
    eta = 2/(sigma * (t+1))
    beta -= eta * gg_full
    beta = np.maximum(np.minimum(beta, beta_max), beta_min)
    # update average and weight sum
    beta_ave = (t * beta + sum_t * g_ave) / (sum_t + t)
    if ( t%(T//20) ==0):
    # if True:
        print("t = {}, beta_ave = {}, B/beta_ave = {}, gg_full = {}".format(t, beta_ave, B/beta_ave, gg_full))
